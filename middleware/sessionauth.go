...
var (
	...
	ContextUserKey string = "context-user"
)
...
func SessionUser(store sessions.Store, sessionName string, newUser func() User) negroni.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request, next http.HandlerFunc) {
		...
		context.Set(r, ContextUserKey, user)

		// HACK: what happens if err?
		next(w, r)
	}
}
...
func LoginRequired(w http.ResponseWriter, r *http.Request, next http.HandlerFunc) {
	// TODO: check the type
	log.Println("testing auth with LoginRequired")
	user := context.Get(r, ContextUserKey).(User)
